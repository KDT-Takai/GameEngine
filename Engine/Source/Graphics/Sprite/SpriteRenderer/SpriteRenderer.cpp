...
//        params[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
        ...
        if (FAILED(hr))
        {
            LOG_HRESULT("SpriteRenderer: RootSignatureの生成に失敗", hr);
            return false;
        }
        ...
        DXGI_FORMAT    dsvFormat)
    {
        ...
        if (FAILED(hr))
        {
            LOG_HRESULT("SpriteRenderer: PipelineStateの生成に失敗", hr);
            return false;
        }
    ...
    bool SpriteRenderer::CreateQuadBuffer()
    {
        ...
        // 頂点バッファ生成
        {
            ...
            if (FAILED(hr))
            {
                LOG_HRESULT("SpriteRenderer: VertexBufferの生成に失敗", hr);
                return false;
            }
            ...
            if (FAILED(hr))
            {
                LOG_HRESULT("SpriteRenderer: VertexBufferのマップに失敗", hr);
                return false;
            }
        ...
        // インデックスバッファ生成
        {
            ...
            if (FAILED(hr))
            {
                LOG_HRESULT("SpriteRenderer: IndexBufferの生成に失敗", hr);
                return false;
            }
            ...
            if (FAILED(hr))
            {
                LOG_HRESULT("SpriteRenderer: IndexBufferのマップに失敗", hr);
                return false;
            }
