...
namespace Engine::Graphics
{
		...
		DXGI_FORMAT dsvFormat)
	{
		...
		{
			LOG_ERROR("Triangle: RootSignatureの生成に失敗");
			return false;
		}
		if (!CreatePipelineState(device, shaderLoader, rtvFormat, dsvFormat))
		{
			LOG_ERROR("Triangle: PipelineStateの生成に失敗");
			return false;
		}
		if (!CreateVertexBuffer(device))
		{
			LOG_ERROR("Triangle: VertexBufferの生成に失敗");
			return false;
		}
	...
	bool Triangle::CreateRootSignature(ID3D12Device* device)
	{
		...
		if (FAILED(hr))
		{
			LOG_ERROR("RootSignatureの生成に失敗 : {0:x}", static_cast<unsigned int>(hr));
			return false;
		}
		...
		DXGI_FORMAT dsvFormat)
	{
		...
		if (FAILED(hr))
		{
			LOG_ERROR("PipelineStateObjectの生成に失敗 : {0:x}", static_cast<unsigned int>(hr));
			return false;
		}
	...
	bool Triangle::CreateVertexBuffer(ID3D12Device* device)
	{
		...
		if (FAILED(hr))
		{
			LOG_ERROR("VertexBufferの生成に失敗 : {0:x}", static_cast<unsigned int>(hr));
			return false;
		}
		...
		if (FAILED(hr))
		{
			LOG_ERROR("VertexBufferのマップに失敗 : {0:x}", static_cast<unsigned int>(hr));
			return false;
		}
