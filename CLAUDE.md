...
**Rendering** is DX12 (`Graphics/DX12/*`: Device / Renderer / RendererContext / DescriptorHeapManager), with two independent draw paths both driven from `Framework::Run()`'s per-frame loop: `SpriteRenderSystem` + `SpriteRenderer` for 2D, `ModelRenderSystem` + `ModelRenderer` for Assimp-loaded 3D models. Each `~RenderSystem` walks an `entt::registry` view and calls the matching `~Renderer::Draw()` — this Component → System → Renderer chain is the pattern to follow for a new drawable type.
...
- `ModelLoader::ProcessMaterial` checks `aiTextureType_DIFFUSE` → `BASE_COLOR` → `EMISSIVE` in that order and reads whichever color property matches — not always `AI_MATKEY_COLOR_DIFFUSE`. FBX exporters (Unity's in particular) don't always put the color map/tint in the legacy Diffuse slot.
